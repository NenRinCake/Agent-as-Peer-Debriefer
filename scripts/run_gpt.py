...
def call_gpt_api(prompt, model="gpt-5", temperature=1):
    """
...
def fetch_data(perspective):
    ...
    for i in range(data_num):
        for j in range(trial_num):
            ...
            code_str = code_prompt.replace("{inputData}", raw_text_data[i]).replace("{researchQuestions}", research_questions[i]).replace("{numberOfTopicClusters}", str(number_clusters[i])).replace("{clusteringStyle}", str(clustering_style[i]))

            # code
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_code_output.json"%(i,j)
            ...
            # code_agent
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_{perspective}_agent_code_output.json"%(i,j)
            code_agent_str = code_agent_prompt.replace("{inputData}", code_response).replace("{researchQuestions}", research_questions[i]).replace("{explanation}", code_explanation).replace("{self_criticize}", code_self_criticize)
            ...
            # sub-theme
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_{perspective}_subtheme_output.json"%(i,j)
            sub_theme_str = subtheme_prompt.replace("{inputData}", code_response).replace("{numberOfTopicClusters}", str(number_clusters[i])).replace("{codingStyle}", str(coding_style[i]))
            sub_theme_response = cache_or_call(output_name, sub_theme_str)
            sub_theme_response, sub_theme_explanation, sub_theme_self_criticize = extract_json_from_str(sub_theme_response)


            # sub-theme_agent
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_{perspective}_agent_subtheme_output.json"%(i,j)
            sub_theme_agent_str = subtheme_agent_prompt.replace("{inputData}", sub_theme_response).replace("{researchQuestions}", research_questions[i]).replace("{explanation}", sub_theme_explanation).replace("{self_criticize}", sub_theme_self_criticize)
            # print(sub_theme_agent_str)
            sub_theme_agent_response = cache_or_call(output_name, sub_theme_agent_str)
            sub_theme_response, sub_theme_agent_modification_summary = extract_json_from_agent_str(sub_theme_agent_response)
            # generate_code.extend(extract_subtheme_names_from_agent_str(sub_theme_response))


            # theme
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_{perspective}_theme_output.json"%(i,j)
            theme_str = theme_prompt.replace("{inputData}", sub_theme_response).replace("{researchQuestions}", research_questions[i]).replace("{conceptualizingStyle}", str(conceptualizing_style[i]))
            ...
            # theme_agent
            output_name = f"../output/gpt5-{datasets_name}-output/data%d_trial%d_{perspective}_agent_theme_output.json"%(i,j)
            theme_agent_str = theme_agent_prompt.replace("{inputData}", theme_response).replace("{researchQuestions}", research_questions[i]).replace("{explanation}", theme_explanation).replace("{self_criticize}", theme_self_criticize)
